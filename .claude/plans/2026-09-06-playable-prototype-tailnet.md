...
An independent review of Steps 15 to 17 is recorded in `docs/reviews/2026-09-06-engine-kernel-review.md`. It confirms that component generation and determinism hold as designed, and finds one latent correctness bug plus a set of DRY defects; each finding is assigned to the step that closes it, and Steps 18 to 21 carry those assignments in their notes.
...
- **Format with `find`, never `git ls-files`.** The gate's formatting check walks `find main.cpp src tests benchmarks -type f \( -name '*.cpp' -o -name '*.hpp' \)`. `git ls-files` omits newly created files, which are untracked at the moment a step formats them, so a step that formats that way ships unformatted new files and fails `native-deterministic-build` on `-Wclang-format-violations`. Format before verifying, so the verified tree is the formatted one.
- **Commit hygiene.** Stage explicit paths, never `git add -A`, while any step's work is in flight: a blanket add sweeps another step's half-finished tree into an unrelated commit, which both lies about what changed and commits unverified code. `test:`, `fix:`, `feat:`, `refactor:`, `build:`, `docs:`, `deploy:` prefixes; every commit stays green on the pull-request profile. Phases 3 to 5 may land as reviewed commits before deployment because the v1 client keeps working against a server that also speaks v2.
...
- [x] **Step 17: Make the tick a fixed kernel with named hook stages**
  ...
  - Execution note (2026-09-06): Verified at 190 tests (185 `unit.simulation`, up from 141) and 366 across all domains. Bit-identity was proved by a second implementation of the accepted seven-phase tick written in the test as an oracle, compared with exact `PhysicsBody` equality over the 1,601-tick pair-fixture horizon, then mutation-tested: perturbing the drag factor by one unit in the last place fails at tick 1. Drag multiplies unconditionally rather than branching on zero, because at zero the factor is exactly 1.0 and multiplication by 1.0 is the identity on every finite value. The index is rebuilt only when the roster actually changed, which keeps the empty-batch path allocation-identical. `TickContext` holds the configuration by value and exposes no clock.
...
- [x] **Step 18: Add the contact rule table, static bodies, and maps as values**
  - Verify: `./scripts/verify-focused 'unit.simulation'`
  - Execution note (2026-09-06): Verified at 243 `unit.simulation` tests (up from 185) and 424 across all domains, with the formatting gate clean. Review findings 1 and 12 are closed here: the grid rebuild predicate now compares what the index is actually a function of, body ids and positions, at three decision points, with a debug assertion at commit that the committed grid equals a fresh rebuild, plus tests for a body created at each hook stage and for a stage calling `destroy_entity` directly; and `ContactEvent` carries an owned fixed-capacity rule name instead of a borrowed view. Routing phase 3 through the table is numerically a no-op because `elastic_disc` delegates to the existing pure equation rather than transcribing it, and the accepted-baseline oracle passes with zero deleted lines. `SimulationConfig` keeps its world fields because protocol v1 and the scenario loader still publish them, while every kernel phase now reads the arena from the map; the map-less overloads synthesize a bare arena from those same scalars so the published and physical arenas cannot disagree. `spatial_index()` during `kPreKernel` exposes the index of the bodies phase 0 left rather than last tick's, so a system can never see a destroyed entity.
  - Notes: `ContactRule` (predicate and response function pointers), `ContactResponse` (two bodies plus events), `ContactRuleTable::built_in()` with dynamic-dynamic elastic and dynamic-static reflect rows evaluated first-match in row order in phase 3; the existing pair tests and the Step 17 bit-identity oracle must pass through the table with identical numbers, and phase 3 gains the layer/mask admission predicate now that `PhysicsBody` carries both. `MapDefinition` (`ArenaBounds`, `static_bodies`, `Marker`s with `spawn` kind, metadata) replaces the world-size configuration as the arena source, and `TickContext` gains `map()`. Also give `TickContext` a read-only `spatial_index()`: ADR 0004 § "The tick" promises post-kernel systems the rebuilt index but no accessor exists, so area queries would otherwise force every mechanic into an O(n) scan. Document its per-stage meaning — last tick's commit during `kPreKernel`, this tick's phase 6 rebuild afterwards. `@extension-point contact_rule`, `@extension-point map_definition`.

- [ ] **Step 19: Add the match lifecycle engine and the `GameMode` interface**
  - Verify: `./scripts/verify-focused 'unit.simulation'`
  - Notes: **Close engine-review findings 2, 3, 4, 14, and 16 first** (`docs/reviews/2026-09-06-engine-kernel-review.md`), because every one of them is load-bearing for a mode: reconcile `GameWorld`'s roster with its component stores so "which entities exist" has one answer; plumb the tick's `EntityIdReservation` into the world so `create_entity()` works and a system can make a projectile, pickup, or zone entity; stop publishing `Controllable::commands_this_tick` in snapshots so an in-process bot cannot read every player's live input for the current tick; apply despawn removals before the commit-time bounds validation so an eliminated body pushed out of bounds is removed rather than stopping the match; and pin `-ffp-contract=off` before any nonzero drag ships. Then add `MatchState`, `MatchPhase`, `MatchOutcome`, `MatchLifecycleDurations`, `MatchObjective`, `SpawnPolicy`, `DeterministicRandom` (SplitMix64 as specified), and the abstract `GameMode` (`name`, `systems`, `contact_rules`, `accepted_command_kinds`, `spawn_policy`, `objective`, `validate_map`) live in `blob_simulation`; the engine-owned lifecycle system runs at `kLifecycle` with at most one transition per tick and writes `MatchSnapshot`. Phase 0 gains spawn seating through the engine `SpawnSystem` and the mode's `SpawnPolicy` over the map's markers, drawing ids from the tick's `EntityIdReservation`, and `GameSimulation::create` takes the map and the mode. Retire `GameWorld::EntitySeed` once spawning seats live entities. Tests use a minimal in-test mode. `@extension-point game_mode`.
...
- [ ] **Step 20: Create `blob_gameplay` with the registry and the `sandbox` mode**
  ...
  - Notes: **Close engine-review findings 5 to 9, 11, 13, and 15 first** (`docs/reviews/2026-09-06-engine-kernel-review.md`), because they are exactly the DRY and discoverability defects this step's seam is supposed to prove: unify `recorded_entity_of` with `addressed_identity_of` so one question has one implementation and a command kind touches the files its registry claims; delete the unmotivated second command ordering; derive `kCommandKinds`, `kWorldEventKinds`, and the rank-injectivity check from the variants instead of hand-typed parallel arrays that pass a size guard while silently dropping a kind; stop `mutable_entries()` exposing `Entry::entity`; add the canonical two-store join ADR 0004 names but never provides; rename `kMaximumPlayerCount` to say entities; and settle on one exception vocabulary. Correct ADR 0004 § "Libraries, and where a new thing goes", whose command-kind row is factually wrong. Then build the library: it depends only on `blob_simulation`; `game_mode_registry.hpp` maps mode names to factories; `SandboxMode` accepts thrust, uses built-in contact rules, seats every spawn at the next marker, never leaves `lobby`-equivalent free play. Its size is the seam's acid test: if it exceeds a few dozen lines, fix the interfaces in ADR 0004 before continuing. Tests mirror `src/gameplay` under `tests/unit/gameplay`.
...
**Amended 2026-09-06:** Assigned the engine-kernel review findings to Steps 18 to 21 rather than adding a remediation step, because each finding's natural home is the step that next touches that code. Steps 19 and 20 now open with their assigned findings.
