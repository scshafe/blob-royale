...
- [x] **Step 8a: Let a body decide how much drag it feels**
  ...
  - Execution note (2026-09-07), commit `3e8d732`. Verified at 841 C++ tests on GCC **and** Clang ASan/UBSan and 4 browser tests. The identity was proved rather than asserted: a hand-written copy of the pre-change phase 1 runs beside the kernel for 1,000 ticks under drag 2.0 and is compared with exact `==`, and `game_simulation_tests.cpp` is a pure addition of 131 lines with **zero** removed, so the accepted oracle is byte-for-byte unchanged. `drag_scale` is unbounded above on purpose: the clamp at zero already makes a large value total, so it saturates at "stops on the tick it is applied", which is a meaningful body rather than an error.
