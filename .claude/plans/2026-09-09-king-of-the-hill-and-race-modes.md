...
- [x] **Step 20c: Repair the tailnet readiness probes**
  - Verify: `bash -n scripts/deploy-tailnet scripts/reconfigure-tailnet && ./scripts/verify-focused 'unit.server|fixtures' && ./scripts/verify-focused 'unit.server|fixtures' linux-clang-asan-ubsan`
  - Notes: The native release at `c5baef0` passed all four 1,133-test suites, web/browser gates, bounded fuzz campaigns, dependency security, and authoritative publication. Deployment then installed files and replaced the container but failed `DEPLOY.NOT_READY`: both tailnet scripts omit `X-Forwarded-For` from their localhost readiness probes, although the deployed policy trusts loopback as a proxy and rejects requests lacking a forwarded client. Fix both callers, add actual readiness-route and caller-contract regression coverage, and align the operations runbook. Preserve the proxy policy, ten-second deadline, and canonical deployment path. The tailnet readiness route already returns 200; the same local request succeeds with `X-Forwarded-For: 127.0.0.1`. A healthy live endpoint does not convert the failed deployment command into success. Push the correction, read its CI result, then rerun the canonical deployment with a fresh log before closing Step 21.
  - Execution note (2026-09-10): Both scripts now supply the canonical loopback forwarded-client header. An actual readiness-route regression proves the headerless 400 and correctly forwarded 200, and a source-contract fixture pins both callers. The runbook explains the request and corrects its stale v1 forwarding statement. Independent root review found the fix scoped to callers, tests, and documentation, with no server-policy/configuration change. Shell syntax and whitespace checks passed; all 149 selected server/fixture tests passed on both GCC and Clang ASan/UBSan, including both new regressions. These local emulated results are advisory; the corrected commit still requires its own CI result and native canonical deployment.

- [x] **Step 21: Deploy the hill and play it**
  ...
  - Continuation note (2026-09-09): Corrected commit `c5baef09c6dc70ce8c7379ae27f683523af591a8` is pushed. Authoritative [CI run 34443634077](https://github.com/scshafe/blob-royale/actions/runs/34443634077) passed all four native builds; all 1,133 tests on GCC, Clang ASan/UBSan, and Clang TSan; 294 web tests; the browser gate; every fuzz regression corpus; and dependency security. The subsequent deployment launcher failed SSH authentication before executing any remote command: the configured SSH agent refused signing, and no existing control connection was available. The live readiness endpoint still reports ready with snapshots available. No native pull, release build, install, or playtest occurred. Resume the canonical deployment at exactly `c5baef0` after the owner unlocks/allows the SSH key agent; do not retry or bypass a failed release gate. Step 21 remains open.
  - Continuation note (2026-09-10): The owner restored SSH access. The native release run published authoritative `release-b3bd38a31482d17189465d7d24becc0c177aead4ce4f643c9a021c366073030f`, then replaced the live container at 08:20:57 UTC with commit `c5baef09c6dc70ce8c7379ae27f683523af591a8`. Its localhost probe returned HTTP 400 / `forwarded_client_absent`, so the script stopped with `DEPLOY.NOT_READY` and exit 1 before reapplying the serve handlers. Independent inspection confirms the running commit/image match the publication, tailnet readiness is 200 with snapshots, the served decoder is 2.5, and all four rooms are healthy compact hill lobbies. A correctly forwarded local readiness request also returns 200. Step 20c repairs the known caller defect; deployment completion and the live match remain pending. Preserve this failed attempt as evidence, not a successful deployment report.
  - Live-check note (2026-09-10): At `c5baef0`, independent comparisons also confirmed installed web/config/map bytes match the publication/checkout and all tailnet handlers are unchanged. One API-only Room 1 match passed at 08:24–08:25 UTC: 894 validated snapshots, five-second countdown, hill movement/scoring, a hill-seeker win at 30 points after 31.31 running seconds, and an eight-second return to lobby. Both temporary NPCs were acknowledged cleared and the synthetic session closed normally. A separate automated Safari Room 2 inspection verified the rendered compact hill, lobby/scoreboard, disabled Start with two empty seats, and protocol 2.5, then left without roster edits. All four rooms subsequently reported healthy, no sessions, and their single configured wanderer. Human feel, keyboard play, running-match UI, knockout/respawn UI, and reconnect behavior were not tested. The playtest note records this scope; the corrected deployment command must still pass before this step closes.
  - Completion note (2026-09-10): Corrected commit `b1e80d41edc9b264ff32eb573510a8b49d69778a` passed authoritative [CI run 34455323059](https://github.com/scshafe/blob-royale/actions/runs/34455323059), whose complete log was read, and the canonical native deployment then passed with `deploy_tailnet.status=passed` and `hill_deploy.exit_code=0`. The new container started at 10:26:59 UTC. Native release verification passed all four builds, all four 1,135-test suites, 294 web tests, seven browser flows, every fuzz regression and six 30-second campaigns, dependency security with zero high/critical findings, and authoritative publication. Independent inspection at 10:28 UTC matched the clean checkout and deployed-commit label to `b1e80d4`, release `release-828bee035be30351a4bd51f1ebd44c7a7ab7a3f5ca24bf13ed8e535b767fe739`, and running image content ID `sha256:f75a2773e0512760c141a88caf2ab90b3440d34380df8814a0a07621d3240f1c` to the publication. Installed web/config/all maps matched their certified or checkout inputs; correctly forwarded localhost readiness and tailnet readiness returned 200; the served decoder is 2.5; all four hill rooms were healthy and advancing; Serve JSON was byte-identical to the predeployment baseline with no Funnel enabled.
  - Final live-check note (2026-09-10): A fresh API-only Room 1 match on `b1e80d4` passed at 10:28:30–10:29:15 UTC: 894 validated snapshots, five-second countdown, hill movement/scoring, a hill-seeker win at 30 points after 31.34 running seconds, and an eight-second return to lobby. Both temporary NPC clear commands were acknowledged, the session closed with code 1000 and no pending cleanup, and a fresh directory read found every room healthy/in lobby with no sessions and its single configured wanderer. The earlier Safari lobby check remains evidence at `c5baef0`; it was not rerun or relabelled as a `b1e80d4` UI check. The completed `docs/playtests/2026-09-10.md` distinguishes these observations from the original failed deployment. Logs: `/tmp/blob-royale-hill-deploy-b1e80d4.log`, `/tmp/blob-royale-deployment-evidence-b1e80d4.log`, `/tmp/blob-royale-served-bundle-b1e80d4.log`, and `/tmp/blob-royale-live-hill-protocol-b1e80d4.log`. Accepted arena/replay baselines remain unchanged from `cd5d90f`, and ADR 0007 remains Accepted. Human feel, live keyboard/running-match/respawn/reconnect UI checks, camera implementation, large-map acceptance, and the later race playtest remain explicit follow-ups, not claims made by this completed compact hill deployment.
...
**Amended 2026-09-10 (tailnet readiness request):** Inserted Step 20c after a fully certified release exposed a localhost probe that violates the existing trusted-proxy policy. Correct the probes and their regression coverage without relaxing server policy or bypassing the canonical release/deployment workflow. The new hill server is independently healthy, but the failed command leaves Step 21 open.
