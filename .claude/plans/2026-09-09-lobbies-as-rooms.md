...
- [ ] **Step 8: Add a royale benchmark case and measure the tick on `cole-ubuntu-pc`**
  ...
  - Execution note (2026-09-09, box left open): The case is in the tree and `scripts/run-benchmarks-linux` passes it `deploy/ubuntu-pc/blob-royale.cfg` and `maps/`, so every number but the seat count is the deployment's own. The window is 6,400 running ticks rather than the 4,000 above: with the deployed five-second countdown the boulder's first spawn tick is 8,000, which a 4,000-tick window never reaches. Run locally in the pinned container on the Mac (Rosetta, GCC 13.3 Release, advisory): 6.9 µs mean, 6.6 µs median, 10.2 µs p99, 22 µs maximum per tick and 0.3 µs per snapshot, every sample hash-matching the reference, recorded in ADR 0006 as a dated advisory amendment. **The remote half of the Verify did not run and the box stays open until it does**: the host closed the SSH connection because the local agent refused to sign for the `ubuntu-tailscale` key ("agent refused operation"), and the host pulls from origin, which this branch has not been pushed to. To finish: push `main`, unlock the agent, run the Verify command above, and add the native numbers to the ADR amendment as its second dated line.
