...
| Inputs | `/srv/blob-royale/config/{blob-royale.cfg,scenario.csv}` (mode 0444) from `deploy/ubuntu-pc/`, mounted read-only at `/run/blob-royale` |
...
3. installs the configuration and scenario under `/srv/blob-royale/config` and syncs the staged
   web deployable to `/srv/blob-royale/web`;
