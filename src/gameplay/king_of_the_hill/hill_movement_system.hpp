...
//
// It writes the `Hill` component and nothing else. The centre it writes is `hill_center` of
...
// is never a participant, and is never wiped. A tick whose reservation is empty cannot create
// anything at all, so that is a hard failure naming the cause rather than a match played with no
// hill and therefore no scoring, which is the rule `zone_shrink` set
