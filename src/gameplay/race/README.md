...
tick of a new match. `course_publisher` stamps a temporary `track`/`track_half_width` wire mirror
from the bound terrain and publishes clocks in every phase without changing standings. Step 8
retires that mirror after terrain reaches clients; no road geometry is authored twice.
