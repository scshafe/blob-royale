...
RaceMode::create(const GameModeConfiguration& configuration) {
  return create(configuration.race, configuration.hazards, configuration.abilities);
}
...
std::unique_ptr<const simulation::GameMode> RaceMode::create(RaceConfiguration configuration,
  ...
  return std::make_unique<const RaceMode>(std::move(configuration), std::move(hazards), abilities);
}
