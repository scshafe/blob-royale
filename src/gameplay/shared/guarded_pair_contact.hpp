...
// contact certified in their orientation. Reuses the accepted baseline/general/static equations,
// quarters each guarded body's received velocity delta, applies simultaneous perfect stops from
// PRE-response incoming motion, then projects guarded closing normal motion away. With no guard,
// the chosen base equation's velocities are preserved exactly, including any rounding residual.
//
...
// correction or a closing residual. Non-contact inputs return unchanged bodies and no facts.
[[nodiscard]] GuardedPairOutcome compose_guarded_pair(
    ...
    const simulation::ContactRule::Subject& second, const simulation::PlayerPairContact& contact,
