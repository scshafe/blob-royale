...
This domain owns the accepted Blob Royale v1 and v2 wire representations. It is the
...
`HttpError`, `V2HttpError`, and `SessionWelcome` values, the `ControllerDirectoryView`
port, and the concrete functions in `protocol_json_encoding.hpp` (v1),
`protocol_v2_json_encoding.hpp` (v2), `command_decoding.hpp` (the inbound direction),
and `protocol_v2_frame_conformance.hpp` (the invariants JSON Schema cannot express).
...
files, that lets the v1 and v2 encoders share one bounded serializer rather than each
...
composition root, logging, or network sessions. Protocol v2 publishes a controller
...
`protocol_json_encoding.hpp` and `protocol_v2_json_encoding.hpp` are the
`@extension-point snapshot_encoding`: another negotiated wire version adds its own
named encoders without changing simulation or publication values. Within v2,
...
`docs/protocol/schema/v1`; v2 must match `docs/protocol/schema/v2`.
