...
template <> struct CommandWireKind<simulation::StartMatchCommand> {
  static constexpr std::optional<std::string_view> value = kV3ClientCommandKindNames[8];
};
...
template <> struct CommandWireKind<simulation::RotateVelocityCommand> {
...
static_assert(kMovementChargeSpeedFractionMaximum == simulation::kMaximumChargeSpeedFraction);
static_assert(kMovementCrossingSpawnRateMaximum == simulation::kMaximumCrossingSpawnRatePerSecond);
